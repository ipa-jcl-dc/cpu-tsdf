...
class TransformProcessor:
    def __init__(self,filename="transform.txt"):
    ...
    def load_from_yaml(self):
        with open("transform.yaml", "r") as info:
            data = yaml.load(info)
            for i in range(36):
                matrix = Transform()
                valid = data["Flag "+str(i)]
                matrix.is_valid= valid =="1"
                matrix_info = data["TRANSFORM "+str(i)]
                matrix.transform = matrix_info["data"]
...
test.load_from_file()
test.best_fit_plane()
test.calculate_flatten_transform()
test.calculate_plane_to_yz_transform()
test.process()
#test.output_to_file()
test.show()
"""
